!function(angular) {
  ...
  var RouteContext = function ( routeProvider ) {
    ...
    this.resource = function ( options, scopedObject ) {
      ...
      var path = resource.scope ? resource.scope + '/' : '';
      ...
      angular.forEach( _.keys( actions ), function ( key ) {
        var action = objectifyAction( actions[key], key ),
            ...
            routeWithoutKey = angular.lowercase( parentPath + parentParam + (angular.isDefined(resource.path) ? resource.path : resource.name) + '/' ).replace('//', '/');
